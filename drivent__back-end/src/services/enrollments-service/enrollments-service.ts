...
export async function createOrUpdateEnrollmentWithAddress(params: CreateEnrollmentParams) {
  const createOrUpdateParamsEnrollment = {
    name: params.name,
    cpf: params.cpf,
    birthday: new Date(params.birthday).toISOString(),
    phone: params.phone,
  ...
  const enrollment = await prisma.enrollment.upsert({
    ...
    create: createOrUpdateParamsEnrollment,
    update: createOrUpdateParamsEnrollment,
  });

  const createOrUpdateParamsAddress = {
  ...
  await prisma.address.upsert({
    ...
    create: createOrUpdateParamsAddress,
    update: createOrUpdateParamsAddress,
  });
}
